...
export const ChatBubble = ({
  ...
  return (
    <div className={cn("flex gap-4 group animate-slide-up", isUser ? "flex-row-reverse" : "flex-row")}>
      {/* Avatar */}
      <div
        className={cn(
          ...
          isUser ? "bg-gradient-to-br from-[var(--primary)] to-[var(--accent)]" : "glass border border-border",
        )}
      >
        {isUser ? <User className="size-5 text-[var(--foreground)]" /> : <Bot className="size-5 text-primary" />}
      </div>
      ...
      <div className={cn("flex flex-col gap-2 max-w-[85%]", isUser ? "items-end" : "items-start")}>
        ...
        <div
          className={cn(
            "rounded-xl p-4 relative text-lg md:text-base wrap-break-word whitespace-pre-wrap",
            isUser
              ? cn(
            ...
            {!isUser && (
              <>
                ...
                <DropdownMenu>
                  ...
                  >
                    {onShare && (
                      <DropdownMenuItem onClick={() => onShare(message.id, message.content)}>
