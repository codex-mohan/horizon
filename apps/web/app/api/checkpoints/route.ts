...
const LANGGRAPH_CHECKPOINTER = path.resolve(process.cwd(), "../backend/.langgraph_api/.langgraphjs_api.checkpointer.json");
    ...
    try {
        ...
        for (const [threadId, checkpoints] of Object.entries(data)) {
            ...
            for (const entry of checkpoints) {
                ...
                existingData.json.storage[threadId][ns][checkpointId] = [
                    encodeBase64Checkpoint(entry),
                ];
            }
        }
...
export async function GET(request: NextRequest) {
    ...
    if (threadId && checkpointId) {
        ...
        const checkpoint = threadCheckpoints.find(
...
export async function PUT(request: NextRequest) {
    try {
        ...
        if (!body.checkpoint || typeof body.checkpoint !== "object") {
            return NextResponse.json(
                { error: "Invalid checkpoint data structure" },
                { status: 400 }
            );
        }
        ...
        if (saveCheckpoints(checkpoints)) {
            ...
            return NextResponse.json(
                { error: "Failed to save checkpoints" },
                { status: 500 }
            );
        }
    } catch (error) {
...
export async function DELETE(request: NextRequest) {
    try {
        ...
        if (checkpointId) {
            ...
            checkpoints[threadId] = checkpoints[threadId].filter(
                (c) => c.checkpoint.id !== checkpointId
            );
