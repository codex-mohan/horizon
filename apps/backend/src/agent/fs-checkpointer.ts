...
export class FileSystemCheckpointer extends BaseCheckpointSaver {
    ...
    constructor(filePath: string = ".checkpoints.json") {
        super();
    ...
    async getTuple(config: RunnableConfig): Promise<CheckpointTuple | undefined> {
        ...
        if (checkpoint_id) {
            ...
            if (threadChecks.length > 0) {
                const last = threadChecks[threadChecks.length - 1];
                return {
                    config: { configurable: { thread_id, checkpoint_id: last.checkpoint.id } },
    ...
    async put(
        ...
        const thread_id = config.configurable?.thread_id;
        if (!thread_id) return { configurable: {} };
