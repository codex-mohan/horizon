...
export const GradientButton: React.FC<GradientButtonProps> = ({
  ...
  const finalWidthStyle = buttonWidth === "full" ? "100%" : typeof buttonWidth === "number" ? `${buttonWidth * 4}px` : buttonWidth
  const finalHeightStyle = buttonHeight === "full" ? "100%" : typeof buttonHeight === "number" ? `${buttonHeight * 4}px` : buttonHeight
    ...
    >
      ...
      <button
        ...
        style={{
          ...
          filter: isHovered && !disabled && variant !== "ghost" ? "brightness(1.1)" : "brightness(1)",
      ...
      >
        {icon && iconPosition === "left" && !isIconOnly && <span className="inline-flex">{icon}</span>}
        {isIconOnly ? <span className="inline-flex">{icon}</span> : children}
        {icon && iconPosition === "right" && !isIconOnly && <span className="inline-flex">{icon}</span>}
      </button>
    </div>
  )
}