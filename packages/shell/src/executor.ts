...
import { $ } from "bun";
import { CommandHistory, type HistoryEntry } from "./history";
...
export class ShellExecutor {
    ...
    async execute(
        ...
        try {
            // Use Bun's shell with proper configuration
            const proc = $`${{ raw: command }}`
                .cwd(cwd)
                .env(env)
                .quiet(); // Suppress default output, we handle it ourselves
        ...
        // Throw error for non-zero exit codes if stderr has content
        if (!success && stderr) {
            throw new ExitError(command, exitCode, stderr, { command, cwd, exitCode });
        }
